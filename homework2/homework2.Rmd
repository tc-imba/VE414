...
So at least someone is guilty, maybe someone in the UEO office, or maybe the instructor, who knows?
...
### (b)
$$f_{postpredict}(x^*)=C\int \Pr(x^*)\cdot f_{post}(y) dy\propto \int_0^1\frac{e^{-y}y^{x^*}}{x^*!}\cdot e^{-(n+\beta)y}y^{\alpha-1+\sum_{i=1}^nx_i}dy\propto\int_0^1e^{-(n+1+\beta)y}y^{\alpha-1+x^*+\sum_{i=1}^nx_i}dy.$$
Let $a=n+1+\beta$, $b=\alpha-1+x^*+\sum_{i=1}^nx_i$,

$$
\begin{aligned}
f_{postpredict}(x^*)&\propto-\frac{y^bay^{-b}\Gamma(1+b,ay)}{a}\Bigg|_0^1\\&=a^{-1-b}[\Gamma(1+b)-\Gamma(1+b,a)]\\&=(n+1+\beta)^{-(\alpha+x^*+\sum_{i=1}^nx_i)}\left[\Gamma\left(\alpha+x^*+\sum_{i=1}^nx_i\right)-\Gamma\left(\alpha+x^*+\sum_{i=1}^nx_i,n+1+\beta\right)\right]\\&=(1.6+n)^{-(9+x^*+\sum_{i=1}^nx_i)}\left[\Gamma\left(9+x^*+\sum_{i=1}^nx_i\right)-\Gamma\left(9+x^*+\sum_{i=1}^nx_i,1.6+n\right)\right].
\end{aligned}
$$
