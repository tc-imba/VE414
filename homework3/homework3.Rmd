...
$$f_{Y=0\mid\mathbf{X}}\propto f_{\mathbf{X}\mid Y=0}\cdot f_{Y=0}=\frac{1}{3}f_{\mathbf{X}\mid Y=0},$$
$$f_{Y=1\mid\mathbf{X}}\propto f_{\mathbf{X}\mid Y=1}\cdot f_{Y=1}=\frac{2}{3}f_{\mathbf{X}\mid Y=1}$$

Since $f_{Y=0\mid\mathbf{X}}$, $f_{\mathbf{X}\mid Y=0}$ are probability distributions, there intergral should both be 1, so
$$f_{Y=0\mid\mathbf{X}}=f_{\mathbf{X}\mid Y=0}=(2\pi)^{-1}(\det\mathbf{\Sigma_g})^{-1/2}\exp\left[-\frac{1}{2}(\mathbf{x}-\mu_g)^T\mathbf{\Sigma_g}(\mathbf{x}-\mu_g)\right]=\frac{1}{2\sqrt{3}\pi}\exp\left[\frac{1}{3}(-139-x_1^2+7x_2+16x_1+x_1x_2-x_2^2)\right].$$

Similarly,
$$f_{Y=1\mid\mathbf{X}}=f_{\mathbf{X}\mid Y=1}=(2\pi)^{-1}(\det\mathbf{\Sigma_t})^{-1/2}\exp\left[-\frac{1}{2}(\mathbf{x}-\mu_t)^T\mathbf{\Sigma_t}(\mathbf{x}-\mu_t)\right]=\frac{1}{2\sqrt{23}\pi}\exp\left[\frac{1}{23}(-207-x_1^2+69x_2+x_1x_2-6x_2^2)\right].$$

The optimal decision rule is
$$\begin{aligned}z=\delta(\mathbf{x})&=\mathop{\arg\min}_{z\in S}\sum_{y\in S}C(y,z)f_{Y=y\mid\mathbf{X}}\\&=\mathop{\arg\min}_{z\in S}\left\{0.1z\cdot f_{Y=0\mid\mathbf{X}}+0.9(z-1)\cdot f_{Y=1\mid\mathbf{X}}\right\}\\&=\mathop{\arg\min}_{z\in S}\left\{0.1z\cdot \frac{1}{2\sqrt{3}\pi}\exp\left[\frac{1}{3}(-139-x_1^2+7x_2+16x_1+x_1x_2-x_2^2)\right]+\right.\\&\qquad\qquad\quad\left. 0.9(z-1)\cdot \frac{1}{2\sqrt{23}\pi}\exp\left[\frac{1}{23}(-207-x_1^2+69x_2+x_1x_2-6x_2^2)\right]\right\}\end{aligned}$$
