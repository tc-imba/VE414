...
Now if $h$ is non-monotone function, which means $f\to h$ is many to one, with the same procedure as above, except that we define the inverse mapping function $h^{-1}(x)$ mapping to either corrrect origin point, then $\tilde{p}$ which maximizes $L(p)$ still corresponds to $h(\tilde{p})$.
